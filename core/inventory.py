...
def build_inventory(html: str, ranges: list[LabelRange]) -> dict[str, object]:
    ...
    qualified_counts = Counter(item.qualified_marker for item in ranges)
    return {
        "components": [item.as_dict() for item in sorted(ranges, key=lambda r: (r.start, r.end, r.marker, r.qualified_marker))],
        "component_counts": dict(sorted(counts.items())),
        "component_variant_counts": dict(sorted(qualified_counts.items())),
