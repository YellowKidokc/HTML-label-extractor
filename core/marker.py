...
BEGIN_RE = re.compile(r"<!--\s*BEGIN:\s*([A-Za-z0-9:-]+)\s*-->")
END_RE = re.compile(r"<!--\s*END:\s*([A-Za-z0-9:-]+)\s*-->")
...
def inject_markers(html: str, ranges: list[LabelRange]) -> str:
    ...
    for position in range(len(html) + 1):
        if position in ends:
            for item in sorted(ends[position], key=lambda r: (r.start, r.qualified_marker), reverse=True):
                parts.append(f"\n<!-- END: {item.qualified_marker} -->")
        if position in starts:
            for item in sorted(starts[position], key=lambda r: (-r.end, r.qualified_marker)):
                parts.append(f"<!-- BEGIN: {item.qualified_marker} -->\n")
