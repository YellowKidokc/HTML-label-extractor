...
class LabelRange:
    ...
    @property
    def qualified_marker(self) -> str:
        return f"{self.marker}:{self.variant}" if self.variant else self.marker

    def as_dict(self) -> dict[str, object]:
        data: dict[str, object] = {
            "marker": self.marker,
            "qualified_marker": self.qualified_marker,
            "start": self.start,
            "end": self.end,
...
def make_label_range(html: str, start: int, end: int, marker: str, pattern: str, variant: str | None = None) -> LabelRange:
    return LabelRange(start, end, marker, pattern, strip_tags(html[start:end])[:120], variant)
...
def element_ranges_by_start_tag(
    ...
    for match in iter_start_tags(html, tag_name):
        tag = match.group(0)
        if predicate(tag):
            ranges.append(make_label_range(html, match.start(), find_matching_element_end(html, match), marker, pattern, variant))
    return ranges


def comment_ranges(html: str, marker: str, predicate: Callable[[str], bool], pattern: str, variant: str | None = None) -> list[LabelRange]:
    return [make_label_range(html, m.start(), m.end(), marker, pattern, variant) for m in COMMENT_RE.finditer(html) if predicate(m.group(0))]
