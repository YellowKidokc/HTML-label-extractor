...
def find_proof_layer(html: str) -> list[LabelRange]:
    return element_ranges_by_start_tag(html, "PROOF-LAYER", lambda tag: has_any_class(tag, ["proof-panel", "proof-pressure"]) or has_attr(tag, "data-component", "proof"), "proof-panel / proof-pressure / data-component=proof")


def find_math_translation(html: str) -> list[LabelRange]:
    return element_ranges_by_start_tag(html, "MATH-TRANSLATION", lambda tag: has_any_class(tag, ["math-translation", "collapsible-math"]) or has_attr(tag, "data-layer", "math-translation"), "math-translation / collapsible-math / data-layer")
...
def find_one_breath(html: str) -> list[LabelRange]:
    ranges = element_ranges_by_start_tag(html, "ONE-BREATH", lambda tag: has_any_class(tag, ["one-breath", "article-summary"]), "one-breath / article-summary")
    for match in START_TAG_RE.finditer(html):
        tag = match.group(0)
